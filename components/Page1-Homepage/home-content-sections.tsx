...
const features = [
  ...
  ["Scam Checker", "Identify potential warning signs in suspicious job offers, recruiters, payment requests, or migration claims.", "Available", "/dashboard/scam-checker"],
];
...
export function ScamCheckerHighlight() {
  ...
  return <section className="border-y border-emerald-100 bg-emerald-50/60"><div className="mx-auto grid max-w-6xl gap-10 px-4 py-16 sm:px-6 sm:py-24 lg:grid-cols-2 lg:items-center lg:gap-16"><div><HomeSectionHeading title="Not Sure About a Job Offer?" /><Link href="/dashboard/scam-checker" className="mt-7 inline-flex h-11 items-center justify-center rounded-md bg-zinc-950 px-5 text-sm font-semibold text-white transition hover:bg-zinc-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2">Check an Offer</Link><p id="scam-checker-note" className="mt-4 max-w-xl text-xs leading-5 text-zinc-600">Risk indicators only — not a legal or fraud determination.</p></div><div className="rounded-xl border border-emerald-200 bg-white p-5 shadow-sm sm:p-6"><p className="text-sm font-semibold text-zinc-950">Check before you trust</p><ol className="mt-5 space-y-2">{checks.map((check, index) => <li key={check} className="flex items-center gap-3 rounded-md border border-zinc-200 bg-zinc-50 px-3 py-2.5"><span className={index === checks.length - 1 ? "flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-100 text-xs font-bold text-amber-900" : "flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-800"}>{index + 1}</span><span className="text-sm font-medium text-zinc-800">{check}</span></li>)}</ol></div></div></section>;
}
